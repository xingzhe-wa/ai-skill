...
8. **Keep project-file scope narrow** — when Hermes dispatches Codex, the execution card must name the exact project files or directories Codex may read/edit. Codex should start from those paths and the specific local references named in the task, not broad project discovery.
9. **Do not scan unrelated project files by default** — prohibit full-project scans of source trees, mapper XML, resources, Maven/Gradle dependencies, generated files, IDE folders, `target`, or build outputs unless the task explicitly requires them. Use `rg` only for a missing symbol/API/pattern that is necessary to complete the task, and keep the search path/file glob constrained to the relevant module/package.
10. **No generic prerequisite wandering inside the repo** — for small edits, Codex should read the project constraints plus the task-specified files, make the scoped change, and run scoped verification. It should not inspect unrelated project docs, dependency declarations, generated artifacts, mapper trees, or adjacent business modules just to "understand the repo".
11. **Execution cards should include a hard stop** — tell Codex to stop and report if the named project files are insufficient, rather than expanding into a broad project scan. Hermes decides whether to broaden scope or dispatch a follow-up worker.
